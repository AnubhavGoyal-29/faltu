...
const getDebateTopic = async (req, res) => {
  try {
    const userId = req.user.user_id;
    const { topic } = await startDebate(userId);
...
const submitDebateArgument = async (req, res) => {
  try {
    const userId = req.user.user_id;
    const { topic, argument } = req.body;
    const debate = await submitDebate(userId, topic, argument);
    res.json({ success: true, debate });
  } catch (error) {
    console.error('🎮 [GAMES] Submit debate error:', error);
    res.status(500).json({ success: false, error: 'Debate submit nahi hui' });
  }
};
...
const getMemeImage = async (req, res) => {
  try {
    const imageUrl = gamesService.getRandomMemeImage();
...
const submitMeme = async (req, res) => {
  try {
    ...
    const battle = await submitMemeCaption(userId, caption);
