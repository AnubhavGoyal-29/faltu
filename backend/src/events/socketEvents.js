  ...
  io.on('connection', async (socket) => {
    ...
    socket.on('send_message', async (data) => {
      try {
        // Validate message
        if (!socket.roomId) {
          return socket.emit('error', { message: 'Not in a room. Join a room first.' });
        }
        
        if (!data || !data.message || typeof data.message !== 'string') {
          return socket.emit('error', { message: 'Valid message required' });
        }
        ...
        // Save message
        const savedMessage = await saveMessage(socket.roomId, socket.userId, messageText);
