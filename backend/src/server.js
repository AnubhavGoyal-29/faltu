...
cron.schedule('0 * * * *', async () => {
  ...
  try {
    // Check AI for event suggestions
    try {
      const aiEvent = await generateCronEventSuggestion({ type: 'lucky_draw' });
      if (aiEvent && aiEvent.shouldHappen) {
        console.log('AI suggested event:', aiEvent.eventType);
        // Could trigger additional events based on AI suggestion
      }
    } catch (error) {
      console.error('AI cron event check error:', error);
    }
