...
const triggerChaos = async (req, res) => {
  try {
    const userId = req.user.user_id; // Use consistent user_id
    const user = req.user; // Already fetched in auth middleware, no need to query again
...
const returnChaosToCreator = async (req, res) => {
  try {
    const returnerUserId = req.user.user_id; // Use consistent user_id
...
const getRecentChaos = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: 'Failed to get recent chaos events' });
  }
};
