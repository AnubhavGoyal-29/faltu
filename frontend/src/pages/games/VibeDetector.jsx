...
const VibeDetector = () => {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true)
    try {
      const response = await api.post('/games/vibeDetector', { vibe_input: vibeInput })
        ...
        <div className="bg-white bg-opacity-90 rounded-3xl p-6 shadow-2xl">
          <div className="space-y-4">
            ...
            <FloatingButton onClick={handleSubmit} disabled={loading} className="w-full bg-cyan-600 text-white py-4 text-lg">
              {loading ? 'Detecting...' : 'Detect Vibe! 🚀'}
            </FloatingButton>
          </div>

          {result && (
            <div className="mt-6 bg-blue-100 rounded-lg p-4">
              <p className="font-bold text-xl">Detected Vibe: {result.detected_vibe}</p>
