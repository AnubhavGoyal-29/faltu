...
const UselessFact = () => {
  ...
  const getFact = async () => {
    setLoading(true)
    try {
      const response = await api.get('/games/uselessFact')
        ...
        <div className="bg-white bg-opacity-90 rounded-3xl p-6 shadow-2xl text-center">
          {!fact ? (
            <FloatingButton onClick={getFact} disabled={loading} className="bg-yellow-600 text-white py-4 px-8 text-xl">
              {loading ? 'Loading...' : 'Get Useless Fact! 🚀'}
            </FloatingButton>
          ) : (
            <div className="space-y-4">
              <div className="bg-yellow-100 rounded-2xl p-6">
                <p className="text-xl font-bold">{fact.fact}</p>
                <p className="text-sm text-gray-600 mt-2">Uselessness: {fact.uselessness_score}/100</p>
